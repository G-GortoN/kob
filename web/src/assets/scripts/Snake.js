...
export class Snake extends AcGameObjects {
    constructor(info, gamemap) {
        ...
        this.direction = -1;  // -1表示无指令，0, 1, 2, 3表示上右下左
        ...
        if (this.id == 1) {
            this.eye_direction = 2;  // 左下角的蛇初始眼睛位置向上，右上角向下
        }
    ...
    update_move() {  
       ...
       if (distance < this.eps) {  // 走到目标点了
            ...
            if (!this.check_tail_increasing()) {  // 如果蛇不变长，砍掉蛇尾
                this.cells.pop();
            }
       } else {
            const move_distance = this.speed * this.timedelta / 1000;  // 每两帧之间行走的距离
            this.cells[0].x += move_distance * dx / distance;
            this.cells[0].y += move_distance * dy / distance;

            if (!this.check_tail_increasing()) {
                const k = this.cells.length;
                const tail = this.cells[k - 1], tail_target = this.cells[k - 2];  // 当前蛇尾和更新后的蛇尾
    ...
    render() {
        ...
        for (let i = 1; i < this.cells.length; i++) {
            ...
            if (Math.abs(a.x - b.x) < this.eps) {  // 如果两个相邻部位在竖直方向
